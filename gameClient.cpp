...
void moveMovingPlatform( movingPlatform& platform,  movingPlatform& platform2, landPlatform& land, landPlatform& land2 ) {
    while (true) {
        std::this_thread::sleep_for(std::chrono::milliseconds(8));
        {
            ...
            platform.collidePlatform(800, land2.getGlobalBounds());
...
int main() {
    ...
    
    {
        ...
        playableRec *playableChar = new playableRec(sf::Vector2f(30, 30), sf::Vector2f(100.f, 150.f));
        ...
        // moving platform - get info from server 
        movingPlatform movingPlat(sf::Vector2f(60, 30), sf::Vector2f(600.f, 450.f));
        // moving platform2 - get info from server 
        movingPlatform movingPlatVert(sf::Vector2f(30, 60), sf::Vector2f(750.f, 170.f));
        movingPlatVert.setX(0);
        movingPlatVert.setY(4);


        // land platform 
        landPlatform landShape(sf::Vector2f(800.f, 800.f), sf::Vector2f(-400.f, 400.f));
        ...
        // land platform 
        landPlatform *landShapeJumpOsb= new landPlatform(sf::Vector2f(200.f, 100.f), sf::Vector2f(400.f, 170.f));
        ...
        std::thread platformThread(moveMovingPlatform, std::ref(movingPlat), std::ref(movingPlatVert), std::ref(landShape),std::ref(landShapeEdge) );
        ...
        //loop through the current playbleCharacters vector, if same id isn't found
            ...
            if(window.hasFocus()){
                    ...
                    if (sf::Keyboard::isKeyPressed(sf::Keyboard::V) && playableChar->getLives() > 0)
                    {
                        sm->runOne("increaseXVal", reload, "object_context");
					    reload = false;
                    }

                    if(playableChar->getPosition().y >= 600.0){
